...
TEST(Movie, CreateManualAndDestroyMovie) {
  ...
  m1->num_actors = 2;
  // char* actors[2] = {};
  // DONE (AREAL): This test was failing so I changed the char array above
  // to the following so it has memory on the heap.
  char** actors = (char**)malloc(m1->num_actors*(sizeof(char*)));
  // DONE·(AREAL)·Added·MallocString.
...
TEST(Movie, CreateFromRow) {
  ...
  // DONE (AREAL) I changed the magic number to a global contant.
  char row[ONETHOUSAND];
  snprintf(row, ONETHOUSAND, "%s", movie_row_A);
